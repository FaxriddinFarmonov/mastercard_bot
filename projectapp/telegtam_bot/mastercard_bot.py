...
#                 "issuing": {"total": 0.0, "debit": 0.0, "credit": 0.0},
#                 "misc": {"total": 0.0, "debit": 0.0, "credit": 0.0},
#                 "reversal": {"total": 0.0, "debit": 0.0, "credit": 0.0},  # yangi qo‘shildi
#                 "grand_total": 0.0,
#                 "grand_debit": 0.0,
#                 "grand_credit": 0.0
#             }
...
#             block, r"Transaction Function:\s*Fee Collection Original/Miscellaneous"
#         )
#         # Yangi section: Reversal/Issuing
#         rev_tot, rev_deb, rev_cred = _extract_section_totals(
#             block, r"Transaction Function:\s*First Presentment Reversal/Issuing"
#         )
#
#         grand_m = re.search(r"Grand Total:\s*([-\d,().]+)", block)
#         grand_deb_m = re.search(r"Grand Total Debit Fees\s*([-\d,().]+)", block)
...
#         results[key]["misc"]["credit"] += misc_cred
#
#         results[key]["reversal"]["total"] += rev_tot
#         results[key]["reversal"]["debit"] += rev_deb
#         results[key]["reversal"]["credit"] += rev_cred
#
#         if grand_m:
#             results[key]["grand_total"] = _to_float(grand_m.group(1))
#         if grand_deb_m:
...
#     ws.title = "MasterCard Report"
#
#     ws.append(["Date", "Acquiring", "Issuing", "Miscellaneous", "Reversal/Issuing", "Grand Total", "Type", "Currency"])
#     ws.append([])
#
#     for (date, currency) in sorted(data.keys()):
#         row = data[(date, currency)]
...
#                    row["issuing"]["total"],
#                    row["misc"]["total"],
#                    row["reversal"]["total"],   # yangi ustun
#                    row["grand_total"],
#                    "Total",
#                    currency])
#
    ...
    for block in blocks[1:]:
        ...
        if key not in results:
            results[key] = {
                ...
                "issuing": {"total": 0.0, "debit": 0.0, "credit": 0.0},
                "misc": {"total": 0.0, "debit": 0.0, "credit": 0.0},
                "reversal": {"total": 0.0, "debit": 0.0, "credit": 0.0},
                "grand_total": 0.0,
                "grand_debit": 0.0,
                "grand_credit": 0.0
            }

        # Har bir bo‘lim uchun qiymatlarni olish
        acq_tot, acq_deb, acq_cred = _extract_section_totals(
            block, r"Transaction Function:\s*First Presentment Original/Acquiring"
        )
        iss_tot, iss_deb, iss_cred = _extract_section_totals(
...
def create_excel(data, output_file):
    ...
    for (date, currency) in sorted(data.keys()):
        ...
        # Total Debit Fees — manfiy (-)
        ws.append(["",
                   -row["acquiring"]["debit"] if row["acquiring"]["debit"] != 0 else 0,
                   -row["issuing"]["debit"] if row["issuing"]["debit"] != 0 else 0,
                   -row["misc"]["debit"] if row["misc"]["debit"] != 0 else 0,
                   -row["reversal"]["debit"] if row["reversal"]["debit"] != 0 else 0,
                   -row["grand_debit"] if row["grand_debit"] != 0 else 0,
                   "Total Debit Fees",
                   ""])
